...
class Config:
    ...
    # 场景生成器版本选择
    # 'v2': 超大圆+大中小圆，航路UAV从超大圆出发，高度分离（研究避撞策略）
    # 'v3': 单圆，所有UAV共用同一大圆，统一高度（研究 UAV 数量对成功率的影响）
    # SCENARIO_VERSION = 'v2'
    SCENARIO_VERSION = 'v3'

    # 场景生成模式选择
    USE_RANDOM_SCENE = True              # 使用随机同心圆场景
    # USE_RANDOM_SCENE = False            # 使用固定场景（测试用）

    # 随机场景参数（仅当 USE_RANDOM_SCENE=True 时有效）
    MAX_CIRCLE_DIAMETER = 100            # 最大圆直径（大圆直径）
    CIRCLE_DIAMETERS = None              # 如为 None，则自动生成 [100, 70, 40]
    # CIRCLE_DIAMETERS = [120, 80, 40]  # 自定义圆直径（单位：米）
    ...
    # 计算方式: waypoint_reward[i] = WAYPOINT_REWARD_COEF * num_waypoints / (i+1)
    
    # 终点/全部到达奖励
    TERMINAL_REWARD_COEF = 2.0           # 终点奖励系数
    # 计算方式: terminal_reward = TERMINAL_REWARD_COEF * num_waypoints
    ...
    # 先运行 migrate_checkpoint.py 生成迁移版本，再填写路径；或置 None 重新训练。
    # RESUME_FROM = None
    RESUME_FROM =  None
    # "models/policy_transformer/random_v3_100x70x40_r6f3/policy_transformer_epoch400.pt"
    ...
    @classmethod
    def snapshot(cls):
        """[新增] 导出当前核心配置为 dict，用于实验记录。"""
        return {
            'POLICY_VERSION': cls.POLICY_VERSION,
            'SCENARIO_VERSION': cls.SCENARIO_VERSION,
            'USE_RANDOM_SCENE': cls.USE_RANDOM_SCENE,
            'NUM_ROUTE_UAVS': cls.NUM_ROUTE_UAVS,
            'NUM_FREE_UAVS': cls.NUM_FREE_UAVS,
            'MAX_CIRCLE_DIAMETER': cls.MAX_CIRCLE_DIAMETER,
            'CIRCLE_DIAMETERS': cls.CIRCLE_DIAMETERS,
            'SELF_STATE_DIM': cls.SELF_STATE_DIM,
            'OTHER_ROUTE_DIM': cls.OTHER_ROUTE_DIM,
            'FREE_UAV_DIM': cls.FREE_UAV_DIM,
            'MAX_FREE_UAVS': cls.MAX_FREE_UAVS,
            'TRAIN_EPOCHS': cls.TRAIN_EPOCHS,
            'STEPS_PER_EPOCH': cls.STEPS_PER_EPOCH,
            'MAX_EP_LEN': cls.MAX_EP_LEN,
            'GAMMA': cls.GAMMA,
            'LAM': cls.LAM,
            'LEARNING_RATE': cls.LEARNING_RATE,
            'COLLISION_PENALTY': cls.COLLISION_PENALTY,
        }
